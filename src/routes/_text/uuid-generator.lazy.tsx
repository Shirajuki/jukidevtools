import { Form, InputNumber, Select, Button, Input, Flex, type FormProps } from "antd";
import { ArrowRightOutlined, CloseOutlined, CopyOutlined } from "@ant-design/icons";
import { useState } from "react";
import { validate } from "uuid";
import generateUuid, { type IUuidVersion } from "../../utils/generateUuid.utils";
...
export const Route = createLazyFileRoute("/_text/uuid-generator")({
	...
	return (
		<InputOutputLayout
			...
			InputView={
				...
				>
					<Form.Item<IOptionTypes> label="Version" name="version" rules={[{ required: true }]}>
						<Select
							options={[
						...
						<>
							...
							>
								<Flex gap="small">
									...
									<Button onClick={generateNamespaceSample}>sample</Button>
								</Flex>
							</Form.Item>

							<Form.Item<IOptionTypes> label="Name" tooltip={"Can be anything"} required={true}>
								<Flex gap="small">
									<Form.Item<IOptionTypes>
										name="name"
										rules={[{ required: true }]}
									...
									<Button onClick={generateNameSample}>sample</Button>
								</Flex>
							</Form.Item>
						</>
					)}
			...
			OutputView={
				<CodeEditor
					code={uuidList.length === 0 ? null : uuidList.join("\n")}
					language="text"
					readonly={true}
				/>
			}
