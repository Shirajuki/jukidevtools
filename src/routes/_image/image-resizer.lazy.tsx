...
import getImageFileDimension from "../../utils/image/getImageFileDimension.utils";
import resizeImage from "../../utils/image/resizeImage.utils";
import { createLazyFileRoute } from "@tanstack/react-router";

export const Route = createLazyFileRoute("/_image/image-resizer")({
...
function ImageResizer() {
	...
	const resize = async () => {
		...
		setOutput({ base64, width: width, height: height });
		dispatch(success());
	};
	...
	const onValuesChange: FormProps<IOptionTypes>["onValuesChange"] = (optionTypes: IOptionTypes) => {
		let values = { ...optionTypes };
		if (options.lockAspectRatio && (values.width === null || values.height === null)) {
			...
			if (
				...
				if (values.width && !values.height) {
					values.height = Math.round(
						(options.originalHeight / options.originalWidth) * values.width,
					);
				} else if (!values.width && values.height) {
					values.width = Math.round(
						(options.originalWidth / options.originalHeight) * values.height,
					);
				} else if (values.width && values.height) {
			...
			InputView={
				...
				>
					...
					<div className="flex gap-4">
						<Form.Item<IOptionTypes> label="Width" name="width" className="mt-8">
							<InputNumber
								min={1}
								precision={0}
								placeholder="auto"
						...
						<Form.Item<IOptionTypes> label="Height" name="height" className="mt-8">
							<InputNumber
								min={1}
								precision={0}
								placeholder="auto"
					...
					<Form.Item<IOptionTypes> name="lockAspectRatio" valuePropName="checked">
