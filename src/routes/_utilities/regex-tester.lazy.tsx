...
import styles from "./regex-tester.module.scss";
...
export const Route = createLazyFileRoute("/_utilities/regex-tester")({
...
function RegexTester() {
	...
	const onFlagChange = (value: string[][]) => {
		...
		compactFlags = compactFlags.sort(
			(flag1, flag2) => compactFlagOptions.indexOf(flag1) - compactFlagOptions.indexOf(flag2),
		);
		options.current = {
			...options.current,
			flags: compactFlags,
			...
			<SingleColumnLayout
				...
				View={
					<Form form={form} layout="vertical" size="large" requiredMark="optional">
						...
						<Form.Item<IOptionTypes> label="Text" required={true}>
				...
				{cheatsheet.map((group) => (
					<div key={group.label} className="mb-4">
						...
						{group.items.map((item) => (
							<div className="flex justify-between items-center my-2" key={item.explanation}>
								<Space size="small">
									{item.expressions.map((expression) => (
										<Typography.Text code key={expression}>
											{expression}
										</Typography.Text>
									))}
								</Space>
